...
public class GPUImage3DLutTableFilter extends GPUImageTwoInputFilter {
    ...
    public GPUImage3DLutTableFilter(InputStream filterInputStream, int dimension) {
        this(filterInputStream, dimension, 1.0f);
    }
    
    public GPUImage3DLutTableFilter(InputStream filterInputStream, int dimension, final float intensity) {
        super(LOOKUP_FRAGMENT_SHADER);
        this.dimension = dimension;
        this.lookupTextureDimension = getLookupTextureDimensionLocation(dimension);
        this.intensity = intensity;
        this.setBitmap(loadLutFromResource(filterInputStream));
    }
    ...
    private Bitmap loadLutFromResource(InputStream filterInputStream) {
        Bitmap result = null;
        if (filterInputStream != null) {
            ...
            for (int bIdx = 0; bIdx < this.dimension; bIdx++) {
                for (int gIdx = 0; gIdx < this.dimension; gIdx++) {
                    for (int rIdx = 0; rIdx < this.dimension; rIdx++) {

                        v = 0xff000000;

                        r = readFloat(filterInputStream, true);// Stay within the RGB limits.
                        g = readFloat(filterInputStream, true);
                        b = readFloat(filterInputStream, true);
                        readFloat(filterInputStream, false);// remove alpha from the stream

                        v |= (int) (0xff * r) & 0xFF;
                        v |= ((int) (0xff * g) & 0xFF) << 8;
                        v |= ((int) (0xff * b) & 0xFF) << 16;

//                        int index = bIdx * dimension * dimension + gIdx * dimension + rIdx;
//                        result.setPixel(gIdx * dimension + rIdx, bIdx, v); // TOKYO
//                        int x = (bIdx / 4) * dimension + rIdx;
//                        int y = (bIdx % 4) * dimension + gIdx;
                        int x = bIdx * dimension + rIdx;
                        int y = gIdx;
            ...
            try {
                ...
                Log.e(TAG, "Can't close filterInputStream:", e);
            }
        } else {
            Log.e(TAG, "Can't close filterInputStream: filterInputStream == null");
        }
        return result;
    }

    private float readFloat(InputStream inputStream, boolean shouldCalculateValue) {
        try {
            if (readBufferIndex + BUFFER_SIZE > readBufferLength) {
                readBufferIndex = 0;
            ...
            Log.e(TAG, "Can't read from filterInputStream:", e);
        }
        return shouldCalculateValue ? Math.min(ByteBuffer.wrap(buffer).order(ByteOrder.LITTLE_ENDIAN).getFloat(), 1.0f) : 1.0f;
    }
}
